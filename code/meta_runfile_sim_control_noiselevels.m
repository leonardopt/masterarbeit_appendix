...
function cfg = meta_runfile_sim_control_noiselevels

scriptdir = pwd;

%% Do struct

do.modelgeneration          = 1;
do.firstlevel               = 1;
do.decoding                 = 0;
do.grouplevel               = 0; % group level decoding
do.parfor                   = 1; % turn off for bugfixing!
do.all_subs                 = 1;
do.similarityanalysis       = 1;
...
cfg.derivative_dir  = fullfile(cfg.all_paths.data_base_dir, 'derivatives_test_sim_control_noiselevels');
...
noise_after_conv    = [0.0 ...
    0.010 ...
    0.020 ...
    0.050 ...
    0.10 ...
    ];
perc_additionalnoise =  [0 ...
    0.01 ...
    0.10 ...
    0.50 ...
    1.0 ...
...
for h_ind = 1:n_hypotheses
    ...
    if contains(sim_name{h_ind}, 'piecewise')
        ...
        % add a very minimal amount of noise to avoid matrix with only NaNs
        % after correlation
%         cfg.saa.data_gen.perc_voxelnoise        = cfg.saa.data_gen.perc_voxelnoise * 10;
%         cfg.saa.data_gen.noise_after_conv        = cfg.saa.data_gen.noise_after_conv * 10;
%         cfg.saa.data_gen.perc_additionalnoise    = cfg.saa.data_gen.perc_additionalnoise * 10;
